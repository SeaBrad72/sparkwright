...
| T8 | **Elevation** — an obligation gate is switched off by its caller | `--record DECOY.md`, `--exclude-globs '*'`, `--stub-pattern` | Closed this release: `--` sentinel + first-assignment-wins fence five gate-defining arguments; universal-glob refusal; exclusion shape assertion | `KIT_OBL_TEST=1` in the **environment** still reopens every fixture flag (boarded `OBLIGATION-TESTMODE-ENV-FLAG`) |
