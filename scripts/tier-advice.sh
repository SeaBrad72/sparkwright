...
# What it changes: Read-only — composes agent-scorecard + dora into a human-facing autonomy-tier decision view; mutates nothing.
# Guardrails: Advisory — adds no classification logic and NEVER actuates (never touches .claude/, the guard, or any tier store); always exits 0; DORA is context, never an input to the recommendation.
set -eu
