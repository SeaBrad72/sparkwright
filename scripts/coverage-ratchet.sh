...
# What it changes: Creates/updates the coverage baseline file (default .coverage-baseline) — seeds it on first run.
# Guardrails: FAILs if coverage regresses below the recorded baseline; rejects non-percent input (exit 2); the floor only moves up when you commit an improved value.
set -eu
