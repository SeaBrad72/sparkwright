...
# POSIX sh; dash-clean.
# What it changes: `new` writes a postmortem stub to <out>/<ID>.md (default postmortems/); `to-backlog` is read-only (emits row stubs to stdout).
# Guardrails: No-clobber on the output file; reads templates/POSTMORTEM-TEMPLATE.md; to-backlog only reads the file it is given.
set -eu
cd "$(dirname "$0")/.."
