...
# What it changes: Read-only — reads a CycloneDX SBOM and reports license-policy violations to stdout; mutates nothing.
# Guardrails: exit 1 on a denylisted (strong-copyleft) license, or on undetermined-under-strict; HONEST — green means declared licenses passed policy, NOT that they are legally cleared.
set -eu
