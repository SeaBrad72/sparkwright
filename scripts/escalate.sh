...
# What it changes: Writes pending escalation records under $KIT_ESCALATION_DIR (default .kit-run/escalations); reads the human verdict file.
# Guardrails: FAIL-CLOSED on a missing/invalid verdict; verdict + ratifier are read from the verdict FILE, never from agent data; unknown trigger -> empty option set -> resolve fail-closes.
set -eu
