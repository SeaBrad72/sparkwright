...
# What it changes: Creates adapters/<harness>/{adapter.json,README.md} from adapters/_TEMPLATE/.
# Guardrails: Rejects a non-slug name and refuses an existing adapters/<harness> (no clobber); run from the repo root; cleans up a partial scaffold on failure.
set -eu
