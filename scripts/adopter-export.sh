...
do_export() {  # <dest> <profile-or-empty>  — returns nonzero on bad dest/profile
  ...
  if [ -f "$_ct" ] && [ -f "$_cr" ]; then
    _tab=$(printf '\t')
    for _c in drift-watch golden-path adopter-export feature-flags-wired containment-audit runtime-security agentops-sensor orchestrator-loop escalation-seam conflict-safe-integration skill-spine; do
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  for p in drift-watch golden-path adopter-export feature-flags-wired containment-audit runtime-security agentops-sensor orchestrator-loop escalation-seam conflict-safe-integration skill-spine; do
