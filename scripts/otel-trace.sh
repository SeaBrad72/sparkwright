...
# What it changes: Writes OTel-semantic NDJSON spans to a pluggable sink (stdout, or the OTEL_TRACE_FILE path when set).
# Guardrails: Reference adapter, not a gate; zero-dep; attribute values are jq-encoded (no untrusted interpolation).
set -eu
