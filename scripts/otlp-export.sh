...
# What it changes: Read-only locally — reads NDJSON spans and POSTs OTLP/JSON to $OTEL_EXPORTER_OTLP_ENDPOINT/v1/traces (skipped with --dry-run); writes no files.
# Guardrails: Opt-in; OTEL_EXPORTER_OTLP_HEADERS are never echoed; all JSON is built by jq (no untrusted value interpolated into shell); --dry-run gives a safe no-network check.
set -eu
...
build_header_config() {
  _h_str="$1"; _h_out="$2"
  # Delimiter-walk, NOT `IFS=,` and NOT `read`: a global IFS assignment trips semgrep
  # (ifs-tampering) and `read` would consume the very newlines the guard below detects,
  # silently splitting a hostile header into two records. Splitting only on ',' keeps the
  # newline guard live; staying in this shell keeps the fail-closed `exit 2` fatal.
  _rest="$_h_str"
  while [ -n "$_rest" ]; do
    case "$_rest" in
    ...
    # An empty field (e.g. 'a=1,,b=2') was dropped by the old IFS split; preserve that.
    [ -n "$_pair" ] || continue
    _key="${_pair%%=*}"
    _val="${_pair#*=}"
    # --- config-injection guard (fail-closed) ---
    [ -n "$_key" ] || { echo "otlp-export: refusing header with empty key (config-injection guard)" >&2; exit 2; }
    case "$_key$_val" in
      *'"'*) echo "otlp-export: refusing header containing a double-quote (config-injection guard)" >&2; exit 2 ;;
    esac
    ...
    case "$_val_check" in
      *"
"*) echo "otlp-export: refusing header containing a newline (config-injection guard)" >&2; exit 2 ;;
    esac
    printf 'header = "%s: %s"\n' "$_key" "$_val" >> "$_h_out"
  done
}
