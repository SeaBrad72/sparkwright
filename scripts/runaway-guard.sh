...
# What it changes: Writes/clears the runaway tally file (default .kit-run/tally); reads the ceiling config (default .kit/budget.conf).
# Guardrails: Fail-closed (exit 2) on a missing/bad config; exit 1 (STOP) when a ceiling is breached; the config is control-plane (agent-immutable), the platform LLM-API cap is the hard backstop above it.
set -eu
