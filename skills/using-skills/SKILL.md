...
<!-- The frontmatter, the discipline phrases, and the six index paths below are conformance-load-bearing:
     ...
     the six index paths (skills/design, skills/plan, skills/tdd, skills/review, skills/worktrees,
     skills/verification). Edits that drop or rename them can turn the skill-spine lock RED. -->
...
## The index — the kit's six spine skills
...
The index names **all six** spine skills. A keystone that forgets one is incomplete — the index is exhaustive by design, and stable because the spine is complete.

## Entry-point honesty (the ceiling)
On the **FLOOR**, this keystone is a **convention the conductor follows**, not an enforced auto-load. The kit owns the discovery discipline and the index completely, but a neutral harness cannot be forced to auto-inject this file at session start — that would be a Claude-Code-specific mechanism, exactly the platform coupling neutrality forbids. So first-contact is documented convention: the Orchestrator / standing session reads this keystone first (see `docs/operations/orchestration.md`) and consults the index before convening the cast. A NATIVE `.claude/` binding may auto-surface it for Claude Code as a bonus. The honest gap: the keystone is *provided and structurally proven* (it exists, indexes all six, and the Orchestrator references it); whether an agent actually consults it at runtime is un-gateable — auto-load is harness-local, not a FLOOR guarantee.
