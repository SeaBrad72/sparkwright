...
**Capture friction at the boundary.** At the close of each operate cycle (postmortem scaffolded, signal fed back to Discover), record any kit friction the cycle surfaced — expected vs. happened, severity, where — to the project's `KIT-FEEDBACK.md` (from `templates/KIT-FEEDBACK-TEMPLATE.md`), stamped with the kit version and vehicle. This is an obligation, not a courtesy: capture it at the moment of friction, because a log reconstructed later loses the cause the operate signal made visible.
