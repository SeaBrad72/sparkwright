...
8. **Pre-push, on the final head:** run `sh conformance/selftest-hermetic.sh --touched` — every touched `--selftest` under both hermeticity faces (stripped env + fresh clone) *before* the push, so an ambient-read defect reds on your tree, not on a runner. (Conduct: the binding face is the CI step; this line is the economy.) **After the push, verify it LANDED by reading the ref back:** `git ls-remote --heads origin <branch>` must print the SHA `git rev-parse HEAD` prints. The pre-push hook and git both write to stderr, so a refusal and a success look alike in a transcript — the forge's ref is the only proof.
9. **Control-plane → authored in a dev-clone, actuated on the recorded GO.** Any task touching control-plane (guard / CI / conformance / claims / agent-or-skill defs / governance markers) is authored in a **dev-clone** (`git clone . <literal temp path>`), where the agent edits directly while **the guard stays armed on the real repo**; the agent **never silently commits control-plane** (never *without* a recorded GO). The human reviews a **CI-green diff**. On the human's recorded **GO** the **agent actuates** the whole path — commit → push → open PR → tag → `scripts/promotion-verify.sh land` (records the GO, confirms the note reached origin, then merges `gh pr merge <ref> --squash --match-head-commit <sha>` — never `--admin`, never `--delete-branch`, leaving the branch intact; see `docs/governance/promotion-contract.md` §land). The human-only solo `gh pr merge --admin` (the kill-switch the guard denies the agent) is retained only as the branch-protection bypass for when a normal squash-merge cannot be satisfied (no second reviewer) — not the ordinary control-plane merge path. Ordinary/team work: the agent merges too, so the human does zero mechanical keystrokes. Version finishing (VERSION bump + README badge + CHANGELOG entry) folds into the slice's own commits so the release step cannot be skipped.
...
| "I'll commit the conformance edit directly to save a round-trip." | Control-plane never lands as a silent agent commit — author it in a **dev-clone** (the guard stays armed on the real repo); the agent actuates (commit → push → PR → tag → `land`) only **on the human's recorded GO**; the human-only `gh pr merge --admin` keystroke is retained solely for the solo branch-protection-bypass case. |
| "I'll just set `KIT_GUARD_SELFEDIT=1` for this one edit." | That is a **global kill switch**, not a scalpel — it disarms destructive-op and secret-read denials too, across the whole session. The dev-clone is the sanctioned route precisely because it leaves the guard **armed**. |

## Terminal state
Every plan task built (each via `skills/tdd` by a **fresh executor per task**, dispatched with a **task brief as a file**), gated by an independent **review between tasks** (builder ≠ reviewer, findings resolved not waived), progress recorded in a **durable ledger** that survives compaction — and since `LOOP-STAGE-ARTIFACT-GATE` (2026-09-04) **that ledger is a file, `docs/reviews/<date>-<row-slug>.md`**, appended round by round, named by a **`Kit-Review:`** trailer on the head commit (comma-separated when a consolidated PR absorbs several slices) alongside **`Kit-Plan:`**; before the push, run the review lane locally, and afterwards the non-author approver simply clicks Approve — they type nothing, and the forge's review requirement is what binds it (`D-240904-2`, `REVIEW-LANE-WAITING-IS-GREEN`) — all tasks integrated, and a final **whole-branch review** passed. Any control-plane change is authored in a **dev-clone** (version finishing folded into the slice) and actuated by the **agent on the human's recorded GO** — commit → push → PR → tag → `scripts/promotion-verify.sh land` (records the GO, confirms the note reached origin, then merges non-`--admin`, branch left intact) — the human doing only the GO. A built, reviewed, integrated branch — with **nothing control-plane silently committed by the agent** (nothing without a recorded GO). Before the human Go/No-Go, if an increment carries a taste-surface (UI/UX/flow/working functionality/table), invoke `skills/demonstrate` — surface the working increment for the human's taste judgment and record the verdict in a UAT sign-off. On the human's recorded GO the terminal handoff is **approve→execute→log**: provide the means and wait, then on the recorded GO actuate the merge/tag (ordinary/team: the agent merges; **control-plane: the agent runs `land`**, human-only `gh pr merge --admin` reserved for the solo branch-protection-bypass case), log the approval (`scripts/promotion-verify.sh record`, folded into `land` for control-plane), and verify `shipped == approved` (`scripts/promotion-verify.sh check`) — `builder ≠ ratifier`. **A slice closes in ONE PR — the merge IS the close, and no close-chore PR is opened for any slice** (`SLICE-CLOSES-IN-ONE-PR`, `D-240903-2`). The board edges:

**Before the push, run `sh conformance/review-lane.sh --pre-push`.** The local run grades **exactly what CI will**: since `REVIEW-LANE-WAITING-IS-GREEN` (2026-09-05) the check is structural — the plan file and the review record the head names — and it reads nothing from the forge in either mode, so a green here is a green there. The **approval** is branch protection's (`required_approving_review_count >= 1` + `dismiss_stale_reviews` + `require_last_push_approval`), and **no run sees it** — not this one and not CI's. The approver types nothing: a plain non-author Approve on the pushed head is the attestation (`D-240904-2`), and the forge enforces it at merge.

| Edge | When | Who |
|---|---|---|
| Ready → In Progress | the row is claimed | agent |
| In Progress → **In Review** | first push — **only if** the slice expects review rounds before the retro can be final; it is no longer a mandatory stop | agent |
| In Progress / In Review → **Done** | the **last local gate is green**, on the push that is expected to merge; the L1 retro is drafted now, with its `Disposition:` line | agent |
| (merge) | the owner's recorded GO | agent actuates (`land` for control-plane) |

**Done on a branch means done-at-merge**: the row is a *proposal* on the branch and *true* on main, because a Done row can reach main only through an approved, green, merged PR. A red CI or a review round adds a line to the retro on the fix push — the row stays Done and the PR stays the same one. A branch that is never merged simply dies with its Done row; there is no cleanup commit. The **Closed** cell is the date of that push; the row cites the **PR link** (the merge SHA does not exist yet, and the link resolves to it on the forge). Every retro closed under this rule carries a **`Disposition:`** naming where its lessons went — `row \`ID\`` / `ruling \`D-YYMMDD-N\`` / `none — <reason>` — and `conformance/backlog-current.sh` fails a retro whose clause resolves to nothing.
