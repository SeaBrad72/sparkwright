...
8. **Pre-push, on the final head:** run `sh conformance/selftest-hermetic.sh --touched` — every touched `--selftest` under both hermeticity faces (stripped env + fresh clone) *before* the push, so an ambient-read defect reds on your tree, not on a runner. (Conduct: the binding face is the CI step; this line is the economy.)
