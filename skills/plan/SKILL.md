...
## Terminal state
A saved, self-reviewed plan at **`docs/plans/<date>-<row-slug>.md`** — tracked, cold-resumable, and **the one convention** since `LOOP-STAGE-ARTIFACT-GATE` (2026-09-04); the two spellings that used to coexist are collapsed. Start it from `templates/PLAN-RECORD-TEMPLATE.md`, and carry the **`Kit-Plan: docs/plans/…`** trailer on the head commit of the PR that builds it: `conformance/review-lane.sh` refuses a sensitive/control-plane PR (and any non-docs-only ordinary one) whose head names no tracked, non-stub plan. The plan's **Design-promised controls** table is the SOURCE of the review record's table of the same name — a design promise with no row here is how a control quietly stops being built. (`docs/superpowers/plans/` remains local scratch and is never what the trailer names.) Handed to the build skill. This skill never starts implementation. On handoff the backlog item moves **`Ready → In Progress`**, linked to the plan (or spec) doc — a link always exists by this point.
