...
- **Control-plane PRs show *"ratification required."*** A PR touching the control plane (the guard, CI, `conformance/`, `adapters/`, the named `scripts/`, or the governing docs) shows the `gate-agent-boundary` check **red** (`exit 1`) until a non-author approval lands — that red *means "ratification required,"* not a broken build. Solo, that's expected — your logged admin-merge above **is** the ratification.
