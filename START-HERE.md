...
- **Control-plane PRs show *"ratification required."*** A PR touching the control plane (the guard, CI, `conformance/`, `adapters/`, the named `scripts/`, or the governing docs) surfaces the `gate-agent-boundary` check as *ratification required* until a non-author approval lands. Solo, that's expected — your logged admin-merge above **is** the ratification; it is not a broken build.
