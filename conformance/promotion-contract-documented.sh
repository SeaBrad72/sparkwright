...
check_file() {
  ...
  # WHERE ACCEPTANCE LIVES. This marker survives CUT-PHASE-GATE-PARK (2026-08-19): the [S1a-i]
  # edit-time phase gate that motivated it was parked, and its companion DIRECTION marker
  # ('edit-time phase gate is prevention, not acceptance') went with it — but the property this one
  # locks was never about that gate. Acceptance stays at MERGE: no pre-merge check, present or
  # future, may be documented as standing in for the recorded GO. That is how an advisory speed bump
  # gets read as the promotion control, and it is exactly the drift this line refuses.
...
A pre-merge check makes no ordering and no ratification claim, so acceptance stays at merge.
...
EOF
  ...
  # The RELATION marker is load-bearing on its own: a doc that loses "acceptance stays at merge"
  # documents some pre-merge check standing in for the recorded GO. (Its former sibling, the
  # DIRECTION negative for the edit-time phase gate, was removed with the gate at
  # CUT-PHASE-GATE-PARK, 2026-08-19 — nothing in the contract describes that gate any more, so a
  # negative dropping its sentence would have been vacuously green.)
