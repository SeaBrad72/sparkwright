...
DEMONSTRATE_SKILL_FILE="${ORCH_LOOP_DEMONSTRATE_SKILL:-skills/demonstrate/SKILL.md}"
...
demonstrate${TAB}name: demonstrate|## When to use|taste-surface|demonstrable increment|frame the specific judgment call|visually-observable-first|verdict recorded in a UAT sign-off${TAB}rev:demonstrate skill not wired to the Reviewer
TABLE
}
...
skill_path() {
  case $1 in
    ...
    demonstrate) echo "$DEMONSTRATE_SKILL_FILE" ;;
    *) echo "" ;;
  esac
}
...
while IFS="$TAB" read -r name markers refs; do
  ...
  rarg=""
  # `IFS= read` is command-scoped -- never a global IFS assignment (semgrep: ifs-tampering).
  # Heredoc-fed, NOT piped: the loop stays in this shell, so `rarg` survives it. This also
  # retires the save/restore/re-set IFS dance the old ';'-split needed inside its own body.
  while IFS= read -r r; do
    [ -n "$r" ] || continue
    dk=${r%%:*}; lbl=${r#*:}
