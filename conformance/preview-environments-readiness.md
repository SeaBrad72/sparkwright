...
> **What the Auto row proves — and doesn't.** `readiness.sh preview-env-ready` confirms the approach is *recorded* (a RUNBOOK §4 "Preview environments:" line). It does **not** verify previews actually spin up, tear down, isolate, or exclude prod data. Those are the **Manual** operator rows. **A green script is necessary, not sufficient.**

## Checklist (blank)
| # | Item | Applies? | Evidence | Check |
|---|------|----------|----------|-------|
| 1 | Preview-env approach recorded (RUNBOOK §4) *(documented)* | | | **Auto:** `readiness.sh preview-env-ready` |
...
> A non-deployable project (library, CLI, batch) marks the whole check **N/A — no deploy surface**; `readiness.sh preview-env-ready` skip-passes it automatically.
