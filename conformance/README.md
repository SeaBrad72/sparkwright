...
| `shellcheck.sh` | script | lints the kit's own shell code at the error/warning floor (`-s sh -S warning`); regression-lock (kit is shellcheck-clean); conditional on shellcheck installed — CI installs it so drift is always caught; `--selftest` verifies clean fixture passes + dirty fixture fails | CI |
