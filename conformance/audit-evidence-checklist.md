...
| Independent code review / SoD | CC1.4, CC6.3 / A.5.3, A.8.31 / PO.3, PW.7 | `builder ≠ reviewer`: a filled `templates/REVIEW-RECORD-TEMPLATE.md` (recorded agent-review + ratification; high-risk adds specific acknowledgments) + the `agent-boundary` gate. **Solo:** the recorded compensating-control bundle (`docs/operations/review-lane.md`), tightening to enforced two-human SoD with a single `enforce_admins: true` flip (zero evidence rework) when a teammate joins. | **Manual** + `sh conformance/review-lane.sh` | |
