...
REQUIRED_IDS="badge-version conformance-ci-wired doc-budget guard-single-source action-pinning security-policy gate-counts drift-watch cost-governance supply-chain-verify gitlab-adoption doctor operate-loop tier-advice named-adapters actionlint-valid template-detectors-aligned provenance-precondition golden-path adopter-preflight adopter-export mode-blind explain feature-flags-wired feature-flags-ready containment-audit token-scope runtime-security structured-logging app-tracing metrics-endpoint otlp-backend agentops-sensor author-not-approver verify-enforced runaway-killswitch version-tag-coherent orchestrator-loop release-tag-on-merge escalation-seam conflict-safe-integration skill-spine"
