...
> **What the Auto rows prove — and don't.** `readiness.sh responsible-ai-ready` confirms the AI System Card is *present, classified, and oversight-named*. It does **not** verify the classification is *correct*, the AI is *fair*, the disclosure *shipped*, or it is *compliant* — those are the **Manual** rows below, signed by the security/compliance owner. The good-citizen lines (prohibited-use ack, data-minimization, review/appeal path) are recommended defaults, **not enforced**. **A green script is necessary, not sufficient.**

## How to use
Produce an `AI-SYSTEM-CARD.md` from `templates/AI-SYSTEM-CARD-TEMPLATE.md`. Items tagged *(documented)* are auto-checkable via `sh conformance/readiness.sh responsible-ai-ready`; items tagged *(verified)* require the owner's judgement/evidence.

## Checklist (blank)

| # | Item | Applies? | Evidence (where/how) | Check |
|---|------|----------|----------------------|-------|
| 1 | `AI-SYSTEM-CARD.md` present *(documented)* | | | **Auto:** `readiness.sh responsible-ai-ready` |
| 2 | US risk classification recorded (consequential / children's / prohibited) *(documented)* | | | **Auto:** `readiness.sh responsible-ai-ready` |
| 3 | Human-oversight mechanism named *(documented)* | | | **Auto:** `readiness.sh responsible-ai-ready` |
...
> A non-AI project (CLI, library, batch job with no model) marks the whole check **N/A — not an AI feature**; `readiness.sh responsible-ai-ready` skip-passes it automatically.
