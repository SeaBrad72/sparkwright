...
> **What the Auto row proves — and doesn't.** `readiness.sh test-data-ready` confirms the approach is *recorded* (a RUNBOOK "Test data:" line). It does **not** verify the data is *actually* synthetic/masked or that no prod data leaked into non-prod. Those are the **Manual** rows. **A green script is necessary, not sufficient.**

## Checklist (blank)
| # | Item | Applies? | Evidence | Check |
|---|------|----------|----------|-------|
| 1 | Test-data approach recorded (RUNBOOK §2) *(documented)* | | | **Auto:** `readiness.sh test-data-ready` |
...
> A pure-compute project (CLI, library, no datastore) marks the whole check **N/A — no test data to manage**; `readiness.sh test-data-ready` skip-passes it automatically.
