...
This stack emits OTel-semantic spans per request (see `src/App/Telemetry.cs`, called by
the app host in `src/App/Program.cs`) that the kit's `scripts/otlp-export.sh` converts to
OTLP/JSON and POSTs to any OTLP backend. Spans, bounded-cardinality Prometheus metrics,
and correlated JSON logs all come from the pure `Telemetry` primitives (`Telemetry.cs`);
the host only wires them to the request lifecycle.
