...
# NOTE: this profile's compose.yaml ships DATABASE_URL as a plain libpq DSN (postgres://...), so no
# scheme rewrite is needed. The `+driver` strip below is a harmless no-op here, kept so the drill also
# works if an adopter points it at a driver-qualified DSN (e.g. an ORM's postgresql+psycopg://...).
