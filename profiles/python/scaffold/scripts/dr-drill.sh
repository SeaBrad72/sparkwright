...
# NOTE: compose.yaml ships DATABASE_URL as a SQLAlchemy DSN (postgresql+psycopg://...); the psycopg
# driver suffix is stripped below so the libpq tools (pg_dump/psql) see a plain scheme.
...
if [ -n "${DATABASE_URL:-}" ]; then
  # Strip any SQLAlchemy driver suffix (e.g. `+psycopg`) so libpq sees a bare scheme it understands.
