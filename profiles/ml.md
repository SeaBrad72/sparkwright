...
scikit-learn / PyTorch · MLflow · DVC · pandera · nbstripout + jupytext + nbqa + nbmake · pytest + pytest-cov · the eval harness (`evals/run.py` — shipped as a deterministic offline reference scorer; swap in the Anthropic SDK as the pinned LLM judge for production, see `evals/rubric.md`) · Evidently (drift) · FastAPI / BentoML (serving) · pydantic + pydantic-settings · structlog + Sentry · Anthropic SDK (`anthropic`). Model tiers (harness-neutral): route routine work to the `fast` tier and hard reasoning to the `deep` tier, each bound to a concrete model in the adopter-owned `.kit/model-map.conf` (e.g. `fast=sonnet`, `deep=opus`); the `fast`-tier model is also the default pinned eval judge unless a project pins another.
