...
#
# What it changes: creates and drops an ISOLATED scratch database (SCRATCH_DB — must end in
#   `_restore_drill`) and writes then removes a temporary dump at $DUMP. It NEVER writes to,
#   drops, or otherwise touches the SOURCE database — it only reads it (pg_dump + two selects).
# Guardrails: fail-closed. Refuses to run if SCRATCH_DB equals SRC_DB, is a protected or empty name
#   (postgres / template0 / template1), or lacks the `_restore_drill` suffix — so a single mis-set
#   env var can never let `drop database` reach live data. The dump is removed afterwards because it
#   may hold Confidential/PII data. Read-only against the source; no network beyond your DB.
set -eu
...
# NOTE: this profile's compose.yaml wires the APP via SPRING_DATASOURCE_URL/USERNAME/PASSWORD (Spring
# Boot's own datasource convention), not DATABASE_URL — this drill is an operational script that talks
# to Postgres directly (psql/pg_dump), independent of how the app connects. Set DATABASE_URL yourself
# when running the drill (e.g. postgres://app:app@localhost:5432/app), or export PG* vars directly.
# The `+driver` strip below is a harmless no-op on a plain postgres:// URL, kept so the drill also
# works if an adopter points it at a driver-qualified DSN.
...
if [ -n "${DATABASE_URL:-}" ]; then
  # Strip any driver suffix (e.g. `+psycopg`) so libpq sees a bare scheme; a no-op on a plain postgres://.
  DBURL="$(printf '%s' "$DATABASE_URL" | sed -E 's#^([a-z]+)\+[a-z0-9]+://#\1://#')"
  # Assign THEN export (shellcheck SC2155): `export X="$(cmd)"` masks cmd's exit status behind
  # export's own, so a failed parse would look like a success.
  PGHOST="$(printf '%s' "$DBURL" | sed -E 's#^[a-z]+://([^:/@]+:[^@]*@)?([^:/?]+).*#\2#')"; export PGHOST
  PGPORT="$(printf '%s' "$DBURL" | sed -E 's#^[a-z]+://([^@]*@)?[^:/?]+:([0-9]+).*#\2#;t;s#.*#5432#')"; export PGPORT
  PGUSER="$(printf '%s' "$DBURL" | sed -E 's#^[a-z]+://([^:/@]+)(:[^@]*)?@.*#\1#;t;s#.*#postgres#')"; export PGUSER
  PGPW="$(printf '%s' "$DBURL" | sed -E 's#^[a-z]+://[^:/@]+:([^@]*)@.*#\1#;t;s#.*##')"
