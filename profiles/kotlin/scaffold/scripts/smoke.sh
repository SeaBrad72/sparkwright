...
#
# Usage:  BASE_URL=http://localhost:8080 sh scripts/smoke.sh   (BASE_URL defaults to :8080)
# Wire it as a `smoke` task. Exits non-zero on ANY failed check so CI/post-deploy gates catch it.
#
...
#
# What it changes: nothing — read-only. Issues HTTP GETs against an ALREADY-RUNNING service at
#   $BASE_URL and asserts on the responses. It starts nothing, writes no files, and mutates no state.
# Guardrails: read-only over HTTP; no writes, no credentials, no destructive verbs. Exits non-zero on
#   ANY failed check so a post-deploy gate fails closed rather than passing a broken deploy.
#
...
#    Strict 'true' parse mirrors src/main/kotlin/com/example/app/FeatureFlags.kt: only the exact
#    string "true" expects the new greeting.
