...
This stack emits OTel-semantic spans per request (see `Telemetry.kt`, called by the
app server) that the kit's `scripts/otlp-export.sh` converts to OTLP/JSON and POSTs to
any OTLP backend. Spans, bounded-cardinality Prometheus metrics, and correlated JSON
logs all come from the pure `Telemetry` object (`src/main/kotlin/com/example/app/Telemetry.kt`);
the server only wires them to the request lifecycle.
...
Set `OTEL_TRACE_FILE` to append spans to a file instead of stdout (handy for capturing a
clean NDJSON stream without grepping container logs).
...
local Jaeger config (`otel-collector.yaml`) above is a dev/CI reference, not a production
backend — it configures no auth because the round-trip is a localhost loop.
