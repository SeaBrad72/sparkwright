...
if [ -n "${DATABASE_URL:-}" ]; then
  ...
  PGHOST="$(printf '%s' "$DATABASE_URL" | sed -E 's#^[a-z]+://([^:/@]+:[^@]*@)?([^:/?]+).*#\2#')"; export PGHOST
  PGPORT="$(printf '%s' "$DATABASE_URL" | sed -E 's#^[a-z]+://([^@]*@)?[^:/?]+:([0-9]+).*#\2#;t;s#.*#5432#')"; export PGPORT
  PGUSER="$(printf '%s' "$DATABASE_URL" | sed -E 's#^[a-z]+://([^:/@]+)(:[^@]*)?@.*#\1#;t;s#.*#postgres#')"; export PGUSER
