...
This stack emits OTel-semantic spans per request (see `src/main/java/com/example/app/Telemetry.java`,
called by the app server) that the kit's `scripts/otlp-export.sh` converts to OTLP/JSON and POSTs to
any OTLP backend. Spans, bounded-cardinality Prometheus metrics, and correlated JSON logs all come
from the pure `Telemetry` primitives (`Telemetry.java`); the server only wires them to the request
lifecycle.
