...
dbt-core (+ warehouse adapter) · Dagster (+ dagster-dbt) · sqlfluff · Great Expectations (or Soda) · pandera · data-diff · pytest + pytest-cov · ruff + mypy · OpenLineage · pydantic-settings · Sentry · Anthropic SDK for any AI features. Model tiers (harness-neutral): route routine work to the `fast` tier and hard reasoning to the `deep` tier, each bound to a concrete model in the adopter-owned `.kit/model-map.conf` (e.g. `fast=sonnet`, `deep=opus`).
