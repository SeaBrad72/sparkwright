...
This stack emits OTel-semantic spans per request (see `telemetry.go`, called by
`server.go`) that the kit's `scripts/otlp-export.sh` converts to OTLP/JSON and POSTs to
any OTLP backend. Spans, bounded-cardinality Prometheus metrics, and correlated JSON
logs all come from the pure `telemetry` primitives (`telemetry.go`); the server only
