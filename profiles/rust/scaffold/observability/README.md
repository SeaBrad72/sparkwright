...
This stack emits OTel-semantic spans per request (see `src/telemetry.rs`, called by
`src/server.rs`) that the kit's `scripts/otlp-export.sh` converts to OTLP/JSON and POSTs to
any OTLP backend. Spans, bounded-cardinality Prometheus metrics, and correlated JSON
logs all come from the pure `telemetry` module (`src/telemetry.rs`); the server only
wires them to the request lifecycle.
