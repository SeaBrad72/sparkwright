...
**How to read it:** the `Responsibility` column is the honest part — **Kit-enforced** (mechanical, automatic evidence), **Kit-assisted** (kit gives the pattern; team produces evidence), **Org-owned** (see [README responsibility boundary](README.md)). Rows that don't apply to a given project are marked **N/A (reason)** — e.g. a service with no personal data marks the Privacy rows N/A. Inside the `Evidence artifact` column, **DECLARED** is the third rung of the kit's honesty axis (enforced / advisory / declared — [`../positioning/sparkwright-overview.md`](../positioning/sparkwright-overview.md)): the checker **ships and runs on demand**, and **no pipeline runs it for you**. An auditor should read DECLARED as *"a repeatable procedure exists and is named"*, never as *"a gate blocks on it"*.
...
| Agent/runtime platform boundary · network-egress allowlist | `docs/enterprise/platform-safety-boundary.md` · `../operations/egress-control.md` | CC6.6, CC6.7 | A.8.20 (networks security), A.8.21 (security of network services), A.8.22 (segregation of networks), A.8.23 (web filtering) | PO.5 | default-deny egress reference + RUNBOOK attestation — **DECLARED**: the checker ships and runs on demand (`sh conformance/egress-policy.sh`); its `--selftest` runs in the kit's own CI, and no pipeline — the kit's or yours — runs the check itself | Kit-assisted |
| Agent/runtime platform boundary · separate prod credentials (SoD) | `docs/enterprise/platform-safety-boundary.md` · `../operations/containment.md` | CC6.1, CC6.3 | A.5.15 (access control), A.5.18 (access rights), A.8.2 (privileged access rights) | PO.5 | break-glass workflow + RUNBOOK attestation — **DECLARED**: the checker ships and runs on demand (`sh conformance/containment-ready.sh`); its `--selftest` runs in the kit's own CI, and no pipeline — the kit's or yours — runs the check itself | Kit-assisted |
| Agent/runtime platform boundary · sandboxed filesystem | `docs/enterprise/platform-safety-boundary.md` · `../operations/containment.md` | CC6.1 | A.8.31 (separation of development, test and production environments) | PO.5 | read-only-mount reference + RUNBOOK attestation — **DECLARED**: the checker ships and runs on demand (`sh conformance/containment-ready.sh`); its `--selftest` runs in the kit's own CI, and no pipeline — the kit's or yours — runs the check itself | Kit-assisted |
| Agent/runtime platform boundary · scoped short-lived tokens | `docs/enterprise/platform-safety-boundary.md` · `../operations/containment.md` | CC6.1 | A.5.17 (authentication information), A.8.2 (privileged access rights) | PO.5 | OIDC->role / short-TTL reference + RUNBOOK attestation — **DECLARED**: the checker ships and runs on demand (`sh conformance/containment-ready.sh`); its `--selftest` runs in the kit's own CI, and no pipeline — the kit's or yours — runs the check itself | Kit-assisted |
