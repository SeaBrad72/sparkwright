...
**Maturity:** the kit is at stage `release-candidate` — judged safe and honest to hand to a stranger: its documented adopter path has been walked, its readiness gate hardened to verify the *real* enforcement surface, its builder-facing machinery exercised on its own CI on every push, and at least one real product built end-to-end on its loop — **Relay**, a deployed dead-man's-switch app (TypeScript-Node profile) whose `builder ≠ reviewer` review caught fail-open bugs the agent had shipped **as passing tests**. Relay is an in-house dogfood, not an external adoption; the remaining gap is the **adopter experience** — no one outside the author has driven the loop. Treat an external pilot's results as the next validation signal — the point at which the kit earns `v1.0.0`. See [MATURITY.md](../../MATURITY.md).
