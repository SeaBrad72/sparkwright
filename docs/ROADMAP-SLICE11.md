...
| **A9** | A | **Re-run the guard red-team + the W2/W3 lens** — adversarially re-test the MCP gate (un-allowlisted MCP tool families, capability spoofing) and confirm the egress/containment conformance is honest (UNVERIFIED, never false-pass); verify W2/W3 moved from HIGH-open to closed-or-honestly-bounded. **This is the arc's exit gate.** | — | ✅ done — W3 closed-in-kit, W2 honestly-bounded; caught 1 blocker (`secret.read` allowed despite A8) → fixed in **11e** (v2.44.0). `superpowers/reviews/2026-06-11-A9-arc-exit-redteam.md` |
| **11e** | B | **`secret.read` remediation** (A9 blocker) — gate the MCP `secret.read` capability (A8 family 6) deny-by-default by action-noun + known secret-store server, with an honest generic-name ceiling. | MINOR | ✅ shipped v2.44.0 — `guard_check_mcp` denies secret reads; corpus + disclosure; escape hatches preserved |

**Arc status: CLOSED (v2.44.0).** W3 → closed-in-kit (MCP capability gate, deny-by-default incl. secret.read, Kit-enforced by name, regression-locked); W2 → honestly-bounded / platform-owned (egress + sandbox/tokens/creds reference shipped + wiring verified three-state; interpreter tail never claimed closed). Carried residual: honor-based attestation (disclosed). The honesty invariant held throughout — no green check implies containment a shell can't deliver.
