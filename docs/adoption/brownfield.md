...
3. Leave `.claude/settings.local.json` alone — it is **gitignored** (personal, per-developer). Do not copy the kit's over yours. **Add `.claude/settings.local.json` to your repo's `.gitignore`** if it isn't already — the kit's own `.gitignore` has the line, but your existing repo's won't, and the personal overrides must never be committed.
   ...
   **Add `.kit-run/` to your `.gitignore` in the same edit.** Same reason, different file: the kit's own `.gitignore` has the line and your existing repo's won't, and `incept` — which would have added it for you — is exactly the step a brownfield adoption skips. `.kit-run/` is the local run directory holding the runaway-killswitch tally and the guard's **deny log** (`guard-denials.ndjson`). The deny log records the offending *segment* of each refused command, redacted for common secret shapes but **not guaranteed clean** (`THREAT-MODEL.md`, "The deny log"), so committing it would publish command residue from every session. It is also append-only and unbounded — see `docs/operations/runtime-guards.md` §Over-deny, "Reading the deny log".
...
6. **The hook's four enforcement dials — you ship OBSERVE, and opt in when you're ready.** The installed hook has four legs that can refuse a push: the **entry declaration** (`KIT_PUSH_DECL` — the pushed head must carry a valid `Kit-*` trailer block), the **design GO** (`KIT_PUSH_GO` — a Sensitive/Control-plane change-set must have a branch-scoped GO record), **citation liveness** (`KIT_PUSH_CITE` — once per push it runs `conformance/citation-live.sh` over your whole tracked tree and names any living-document `file:line` citation whose target line has vanished or gone blank), and **board presence** (`KIT_PUSH_PRESENCE` — a Sensitive/Control-plane push whose board, *at the pushed commit*, has no row bearing the branch name in a `PR` cell; it names the rows sitting In Progress and the exact edit, and never writes the board itself). All four **observe** on your tree: they print and allow. That is deliberate — your first push is never red, and the kit's own dial state is `export-ignore`d so it can never arrive with the copy. ⚠️ **These hook dials are separate from `LOOP_STATE_MODE`,** the merge-time dial in `.github/workflows/adopter-gates.yml`, which ships `enforce` (since 2026-08-30). The hook defaults stayed at observe deliberately: flipping a *shipped hook default* would leak enforce at every hook re-copy, which is exactly what `D-240811-2.1` forbids. So your first *push* is still never blocked locally, while your first *merge* is gated — and what that gate asks for is proportional to the change class (ordinary: `Kit-Row` + `Kit-Class`).
