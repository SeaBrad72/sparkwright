...
| **9d-b** | B | **Runtime-guard portability** (R4 cont.) — extract the guard deny-matrix into a runtime-agnostic core + one second-runtime reference (generic pre-exec/pre-push hook) so non-Claude runtimes inherit the destructive-action denials. **Split from 9d** — edits the control-plane `guard.sh`, so human-gated at the terminal (`KIT_GUARD_SELFEDIT`). Own spec before any guard change. | P1 | MINOR |
