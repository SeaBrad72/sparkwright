...
> **`codex` is now the one named adapter whose floor is *exercised* end-to-end, not merely declared.** In the CP-7 cold field test (kit `3.173.0`, vehicle `codex-pulse-6`, operator-run, no Claude assistance), Codex drove the surfaces live: the emitted `pre-push` guard refused a genuine six-commit `git push origin HEAD:main` (exit 1, `origin/main` unchanged), the caller-agnostic `control-plane-ratification` gate held a real control-plane PR blocked while GitHub rejected author self-approval, and the emitted required `ci` path was green on first run yet reddened and blocked a disposable negative PR. All five CP-7 acceptance criteria passed non-vacuously; the full AC-by-AC record is retained in the maintainer-internal design docs (`docs/architecture/2026-07-22-cp7-codex-recert-field-test-evidence.md`). This promoted `codex` from `experimental` to **floor-verified** in the maturity surface. It remains floor-verified, not first-class: Codex has no inline `PreToolUse`-equivalent interception, and AC1 is the test that it discloses so.
