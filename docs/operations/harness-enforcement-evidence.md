...
**Dogfooded (H1.4 + D4 + CP-9):** the kit's own `.github/workflows/ratification.yml` runs the real `gate-agent-boundary` job on every PR **and on every review** (an approval re-evaluates the gate, and only the gate — which is why it has its own workflow file). **D4** presents the result as a **merge-gate**: the job exits 0 and posts a distinct `control-plane-ratification` check-run — **`in_progress` with no conclusion (unratified: yellow, still blocking)** / `success` (ratified) / `failure` (**only** a genuine gate error) — required in branch protection, so an unratified control-plane change is blocked pre-merge **without a red ❌ or a "run failed" email.** **CP-9** replaced the old `action_required` unratified state: see the corrected live evidence below.
...
**Live evidence (probe PR #305, 2026-07-13 — the CP-9 mechanism, witnessed not assumed):** a required check held at **`status: in_progress`** (conclusion `null`) **blocks** — with a non-author approval already present and every other required check green, `mergeStateStatus` was **`BLOCKED`**; when the gate re-read the approval and flipped to `success`, it became **`UNSTABLE`** (mergeable). Same PR, same SHA, same reviews — only the check moved. And it renders **non-red**: read cold and unprompted as "still running", not "failed". ⇒ enforcement is preserved natively, with **no branch-protection change**, and red is freed for genuine errors. **Honest ceiling:** this pins *today's* rendering. The kit controls the status it posts, not the pixels. You clear it with a non-author approval (the check flips to `success`/green) or, solo, a logged `enforce_admins:false` admin-merge — so red ❌ / failure emails are reserved for *genuine* failures. (We also found the prior `gate-agent-boundary` was never a *required* check — it rendered red + emailed but didn't actually block; D4 replaced noise-without-enforcement with enforcement-without-noise.) Adopters: add `control-plane-ratification` to your `main` required status checks.
