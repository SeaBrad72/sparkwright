...
The Orchestrator wears the Product hat and follows the kit's own `skills/continuous-discovery/SKILL.md` for problem-space product discovery (outcome over output; surface + test the riskiest assumption before solutioning) ahead of the Architect hat, then follows the kit's own `skills/design/SKILL.md` then `skills/plan/SKILL.md` for design/planning (the Architect hat), the Engineer follows the kit's own `skills/tdd/SKILL.md` for test-driven build and the kit's own `skills/debugging/SKILL.md` for root-cause debugging (reproduce the bug as a red→green regression test before fixing) and the kit's own `skills/evals/SKILL.md` for eval-driven development of any model/prompt-dependent behavior (with the Security-reviewer applying its red-team/safety lens as the §7 AI security gate), and the Reviewer follows the kit's own `skills/review/SKILL.md` for code review, and the Orchestrator follows the kit's own `skills/worktrees/SKILL.md` for isolation, and the Engineer and Orchestrator both follow the kit's own `skills/verification/SKILL.md` for verification-before-completion (evidence-before-claims + confabulation-proofing), and — the discovery keystone — the Orchestrator/standing session consults the kit's own `skills/using-skills/SKILL.md` first to find the right skill (its `using-superpowers`-equivalent, indexing the spine skills), and — at loop-close — the Orchestrator wears the Operations hat and follows the kit's own `skills/operating/SKILL.md` for handling live signals safely (blast-radius → tier → advisory/escalate → loop-close back to Discover) — bricks #1-11 of the kit's own skill spine (evals, continuous-discovery, and operating are kit-originals; superpowers has none of these crafts). Entry-point convention: on the FLOOR the Orchestrator/session reads `skills/using-skills/SKILL.md` first as a convention the conductor follows (invoke-by-read); a NATIVE `.claude/` binding can auto-surface it for Claude Code, but auto-load is harness-local, not a FLOOR guarantee.
