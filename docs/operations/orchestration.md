...
The Orchestrator follows the kit's own `skills/design/SKILL.md` then `skills/plan/SKILL.md` for design/planning (the Architect hat), the Engineer follows the kit's own `skills/tdd/SKILL.md` for test-driven build, and the Reviewer follows the kit's own `skills/review/SKILL.md` for code review, and the Orchestrator follows the kit's own `skills/worktrees/SKILL.md` for isolation — bricks #1-5 of the kit's own skill spine.
