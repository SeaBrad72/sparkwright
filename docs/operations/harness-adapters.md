...
> The "Verified by" column spans both the floor verifier (e.g. `conformance/guard-core-sourced.sh`, run for every harness) and the native-proof verifier (e.g. `conformance/guard-wired.sh`, `conformance/mcp-policy.sh`, run only when `level` is `"native"`); the manifest-schema section below documents which script fills which role. Those verifiers run in the kit's own CI and ship to you to run on demand — no emitted pipeline wires them.
...
- **`controlPlanePaths`** — non-empty array declaring the control-plane surface this harness can modify (the guard + its config, CI, CODEOWNERS, the harness's own settings). The `agent-boundary` gate **enforces the union of these paths across all adapters**, in addition to the kit-standard `guard-core.sh::is_control_plane_path` floor: an unratified PR touching any declared path fails the gate. (For example, the `generic` adapter declares `AGENTS.md` — `AGENTS.md` is already inside the guard-core set (`_CP8B_GLOB_LEAVES` in `.claude/hooks/guard-core.sh`), so the adapter's declaration is belt-and-suspenders over a path the guard already protects unconditionally, not coverage of a gap.) Entries are matched exactly or as a directory prefix (a value ending in `/`). List every control-plane file the harness can touch so the gate — and a human reviewer — protect the complete set.
...
- **`contextFile`** — **REQUIRED** string naming the single document *this harness auto-loads at session start*. Not "a file the harness could read" and not "the file we wish it read" — the one it actually loads. This is the field that makes `context-binding` mean something: `agents-brief.sh` asserts only that `AGENTS.md` exists, routes to the canonical docs, and stays within a line bound; it never asks **which file the harness loads**. That gap is how `adapters/claude-code` conformed while binding to `.claude/settings.json` and naming no context document at all.
  ...
  | `codex` | `AGENTS.md` | `adapters/codex/README.md`, the *"Adopting with Codex"* section — loads `AGENTS.md` from the repo root on the first turn; plus the cold field test, whose AC-by-AC record is archived on the history ref `history/archaeology-2026-08` (`git show origin/history/archaeology-2026-08:docs/architecture/2026-07-22-cp7-codex-recert-field-test-evidence.md` — the `origin/` prefix is required; `git fetch origin history/archaeology-2026-08` first if your clone predates the archive; the ref exists in the kit maintainers' repository only — the public mirror and adopter trees do not carry history branches; full recovery convention in `docs/kit-internals/retiring-conventions.md` §8) | **measured** |
