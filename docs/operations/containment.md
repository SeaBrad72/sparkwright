...
- **Headless agent sandbox** — the `agent` service in `profiles/<stack>/compose.yaml` — the `typescript-node` reference is `profiles/typescript-node/compose.yaml`, and every other container profile ships the same service (code spans, not links: incept prunes the profiles you did not choose, so a link here would break your first `check-links` run): `read_only` root, `tmpfs` scratch, **work-tree-only** mount, `cap_drop: [ALL]`, `no-new-privileges`, and `network_mode: none`. Opt-in (`docker compose --profile agent run --rm agent`); a plain `docker compose up` never starts it, so the verified app path is untouched. This is the **strong** reference — genuinely host-isolated and no-network.
...
- **IDE sandbox** — `profiles/<stack>/.devcontainer/devcontainer.sandbox.json` (the `typescript-node` reference: `profiles/typescript-node/.devcontainer/devcontainer.sandbox.json`): the same `--read-only`/`--tmpfs`/`--cap-drop` hardening for a Dev-Containers workflow.
