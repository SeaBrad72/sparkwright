...
## The general kill-switch posture (a circuit-breaker, not a gate)
A **kill-switch** is a human keystroke retained at a **high-assurance actuation point** *purely* as a circuit-breaker against a compromised / malfunctioning agent — the same object whether it wears the "deploy-failsafe dial" label (production deploy) or the "human merges the control-plane" label (solo governance). It is calibrated to **blast-radius × threat-model**, and it is explicitly **not a validation** (the GO is the validation) and **not a gate** (a gate would key on the process mode — forbidden by `conformance/mode-enforcement-blind.sh`). It is a documented, human-configured **operational posture**.
...
- **It composes with rollback, above:** the kill-switch stops a *bad actuation* from happening; the rollback order undoes one that shipped. The circuit-breaker is the cheapest of all — it never widens in the first place. See the model in [`../governance/promotion-contract.md`](../governance/promotion-contract.md) ("The general kill-switch posture").
- **Honest ceiling:** a posture, not a wired mechanism — no live prod-deploy consumer yet; wiring the actuation rides a later slice (S5/S6 + KW23).
