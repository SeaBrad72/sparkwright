...
- **D1b — scheduled go/no-go-lite (NEXT, fast-follow)** — a cron workflow that runs the conformance suite + `claims-registry.sh` + link-integrity on a timer and surfaces drift (catches drift *between* commits + is the home for semantic checks too heavy per-push). Feeds / overlaps `sparkwright doctor` (P3).
