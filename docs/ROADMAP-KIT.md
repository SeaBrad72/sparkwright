...
- **(7) `prototype`≡`team` mode-dial dead-letter (C1)** *(OPEN — confirmed false-promise since M1).* `incept.sh` has a single `prototype|team)` branch with identical behaviour; only `enterprise)` differs. Resolve before E3 multi-agent work: either genuinely lighten `prototype` or collapse to honest `team`/`enterprise` labels. **Needs a direction decision (brainstorm).**
- **(1) Widen golden-path path-filter (F7)** *(partial).* `golden-path.yml` `paths:` omits `conformance/verify.sh`, `conformance/claims-registry.sh`, `conformance/check-links.sh`, `scripts/new-profile.sh` — golden-path can skip when those change. Small/mechanical.
- **(5) Private-repo `enforce_admins` honesty note** *(open, tiny).* Document (in `branch-protection.sh` header / `review-lane.md`) that `enforce_admins` 404s on free-tier private repos — the real reason T2-team-live is provisioning-blocked.
