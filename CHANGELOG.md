...
### Added
- **E5-trace — app-level request tracing on the reference app (zero-dep OTel spans).** The reference `server.ts` now emits, per request, an **OTel-semantic span** in the exact `scripts/otel-trace.sh` schema (`trace_id`/`span_id`/`name`/`start_unix_nano`/`end_unix_nano`/`attributes`/`status`), which the existing `scripts/otlp-export.sh` converts to OTLP/JSON — applying the kit's own agent-telemetry idiom to app HTTP requests. **Zero-dependency** (`node:crypto` `randomBytes` for the 32-hex trace / 16-hex span ids; `node:fs` for the optional `OTEL_TRACE_FILE` sink, else stdout). The span carries a **real duration** (wall-clock anchor + `process.hrtime` delta — improving on the agent spans' documented zero-duration), `http.request.method` / `http.response.status_code` attributes, and a `request_id` attribute that **correlates to the E5-log structured line**. The span `name` strips the query string (cardinality + secret hygiene). Proven **behaviourally** by a new `golden-path` step that asserts the booted container emits a schema-valid span (32/16-hex ids, `end > start`, attrs) **and** that it converts through the real `otlp-export.sh --dry-run` seam (non-vacuous: the pre-trace app emitted no span), and **statically** by a new `conformance/app-tracing-wired.sh` lock + claim `app-tracing` (mirrors `structured-logging`/`runtime-security`; `--selftest` good/bad fixtures load-bearing). Kit-self (carved from the adopter export — the adopter receives the span-emitting scaffold, not the kit's golden-path lock). Right-weighted: no new gate type (extends golden-path + the claims registry; reuses the existing OTLP seam). Honest ceiling: proves zero-dep span **emission + OTLP-convertibility** on the reference app — a single root span per request; child spans / cross-service context propagation / auto-instrumentation are the adopter's (swap in the OpenTelemetry SDK), and a live OTLP backend round-trip is the later E5-backend slice. Second slice of epic E5. Design: `docs/architecture/2026-06-29-e5-trace-request-spans-design.md`.
...
### Changed
- **Pre-E10 hardening — guard two-matcher symmetry + tag-time CI gate.** Two banked control-plane hardening items from meta-control panels #16–#18, shipped as one slice ahead of E10. **(A) Guard symmetry:** the verifiers in `conformance/` (and the `adapters/` registry) were protected on the Write/Edit + path-extraction route (`is_control_plane_path`) but **not** on the raw-shell command matchers — a `> conformance/x.sh` / `sed -i conformance/x.sh` redirect could slip the command regex. Added `conformance/` + `adapters/` to both `guard-core.sh` shell matchers (the `:82` mention regex and the `:85` redirect regex), closing the redirect-form gap with two-matcher symmetry; proven by new `conformance/agent-autonomy.sh` deny/allow cases (the redirect/`sed -i` forms now DENY, reads still ALLOW). Honest ceiling: like the existing `skills/` token and the Write/Edit path route they mirror, the new `conformance/`/`adapters/` tokens match the directory name anywhere in a path (e.g. `src/adapters/`) — intentional fail-safe over-blocking, symmetric with the established matcher, not a new bypass. **(B) Tag-time CI gate:** `scripts/release-tag.sh` now refuses to tag a commit whose **main CI definitively failed** (the brick-#8 hole — a red-`conformance` commit got tagged because `--admin` merge bypassed branch protection and the release script only checked VERSION/tag coherence). The gate is **bounded-poll** (waits for the head commit's CI to conclude, since the tag fires while CI is still in-progress) and **degrades open** — it warns and proceeds on no-`gh` / non-GitHub / no-run / poll-timeout, preserving the script's pure-git forge-neutrality. The CI lookup is an **injectable probe** (`RELEASE_TAG_CI_PROBE`; `RELEASE_TAG_CI_TIMEOUT`/`_INTERVAL` bound the poll) so `release-tag.sh --selftest` proves all four branches (failure→refuse, success→proceed, in-progress-timeout→proceed, no-signal→proceed) with **no network**. Honest ceilings: the guard is a speed-bump (closes the shell redirect form, not an interpreter bypass); the tag-gate is a backstop that degrades open by design (a CI slower than the poll, or a forge without the probe, is not blocked). Right-weighted: no new gate/claim/seat — extends `agent-autonomy.sh` (run by `verify.sh`) and `release-tag.sh --selftest` (run by `release-tag-wired.sh`). Design: `docs/architecture/2026-06-28-pre-e10-hardening-design.md`. Plan: `docs/architecture/2026-06-28-pre-e10-hardening-plan.md`.

## [3.67.0] — 2026-06-28

### Added
- **Skill-spine brick #10 — the kit's own `continuous-discovery` skill** (`skills/continuous-discovery/SKILL.md`). The kit now ships its own harness-neutral **problem-space product-discovery** craft — the front of the loop and the partner to `skills/design/SKILL.md`'s solution-space, a **kit-original** (superpowers has no discovery skill; its `brainstorming` is solution-space = the kit's `design`). The reframe is the human↔AI **discovery partner**: the human is the PO and the agent never decides what to build — it structures the space and keeps it honest. The craft anchors on **outcome over output**, builds the **opportunity solution tree** with the human, surfaces and ranks the **riskiest assumption**, and designs the smallest test (a **small bet**) that would prove or kill it before a slice is built. It POINTS AT the existing discovery infra (`templates/FEATURE-REQUEST-TEMPLATE.md`, the Definition-of-Ready success-metric/hypothesis item, `conformance/discovery-complete.sh`) and hands off to `skills/design/SKILL.md` rather than duplicating them. Wired **single-seat** to the **Orchestrator** (a new **Product hat**, before the Architect hat; `agents/orchestrator.agent.md` + `.claude/agents/orchestrator.md`) — no Product *seat*, because the human owns the decision (agents-vs-skills). FLOOR-only (invoke-by-read). Named **`continuous-discovery`**, not bare `discovery`, to keep one-term-one-meaning with the skill-discovery keystone. **This completes the intended Skill-Spine Phase 2** (debugging → evals → continuous-discovery), toward **E10 (build a real slice using only the kit's own roster + skills, zero superpowers)**. Right-weighted: no new gate — claim `skill-spine` extended (kit-original wording: bricks #1–8 replace superpowers, `evals` and `continuous-discovery` add crafts it lacks) + the wiring extends `orchestrator-loop-wired.sh` (a new `check_discovery_skill` asserting the skill + six kit-distinctive markers [`name: continuous-discovery`, `discovery partner`, `outcome over output`, `opportunity solution tree`, `riskiest assumption`, `small bet`] + the Orchestrator reference, with non-vacuous teeth: case 24 [marker teeth — drop `outcome over output`] + case 25 [Orchestrator omits the reference]; the structural `check_keystone` now also requires `skills/continuous-discovery` indexed). Folds in the banked count-neutral sweep at the orchestrator defs (stale "six spine skills"). Design: `docs/architecture/2026-06-28-continuous-discovery-skill-design.md`. Plan: `docs/architecture/2026-06-28-continuous-discovery-skill-plan.md`.

## [3.66.0] — 2026-06-28

### Added
- **Skill-spine brick #9 — the kit's own `evals` skill** (`skills/evals/SKILL.md`). The kit now ships its own harness-neutral **eval-driven-development** craft — the AI-native sibling of `skills/tdd/SKILL.md` and a **kit-original** (superpowers has no evals skill, so this *adds* the AI-native discipline rather than replacing one). The distinctive framing is **probabilistic red→green**: write the eval (dataset + rubric/`judge` + `threshold`) FIRST, watch the *unbuilt* feature **miss the `threshold`**, then build to the bar — with "green" defined as a `threshold` (score ≥ bar; no metric drops > N pts vs a pinned baseline), **not** 0 failures, and the `tdd` non-vacuity law made probabilistic (an eval the unbuilt feature already passes proves nothing — calibrate it to discriminate). It pins an **independent `judge`** (no self-grading; SUT + judge model/version pinned), maintains a **`red-team`** safety set (jailbreaks, prompt-injection, harmful-output; AI incidents become permanent cases), insists runtime guards (prompt-injection defense + output-schema validation) **complement** dev-time evals, and states the **author-don't-run-the-live-provider** policy (the key speed-bump is deliberate). It POINTS AT the kit's existing eval infra (`templates/EVAL-PLAN-TEMPLATE.md`, `conformance/eval-ready.sh`, the §7 Eval gate, `templates/AI-SYSTEM-CARD-TEMPLATE.md`) and chains to `skills/tdd/SKILL.md` + `skills/verification/SKILL.md` rather than duplicating them. Wired **DUAL-SEAT, both asserted**: the **Engineer** references it for eval-driven build (`agents/engineer.agent.md` + `.claude/agents/engineer.md`) and the **Security-reviewer** references it for the red-team/safety/judge-independence lens — the §7 security gate for AI (`agents/security.agent.md` + `.claude/agents/security-reviewer.md`). FLOOR-only (invoke-by-read; no adapter dimension). Second brick of **Skill-Spine Phase 2** (debugging → evals → discovery), toward **E10 (build a real slice using only the kit's own roster + skills, zero superpowers)**. Right-weighted: no new gate — claim `skill-spine` extended (kit-original wording: bricks #1–8 replace superpowers, `evals` adds the AI-native craft it lacks) + the wiring extends `orchestrator-loop-wired.sh` (a new `check_evals_skill` asserting the skill + the five kit-distinctive markers [`name: evals`, `eval-driven`, `judge`, `red-team`, `threshold`] + BOTH the Engineer and Security references, with non-vacuous teeth: case 21 [marker teeth — drop `red-team`] + case 22 [Engineer omits the reference] + case 23 [Security omits the reference]; the structural `check_keystone` now also requires `skills/evals` indexed). Folds in **panel-#16 Low-1** (count-neutral keystone prose: "every spine skill on disk" replaces "all seven / seven spine skills"). Design: `docs/architecture/2026-06-28-evals-skill-design.md`. Plan: `docs/architecture/2026-06-28-evals-skill-plan.md`.
