...
| **SAST** *(first-party code)* | Do static-analysis findings pass — no high-severity injection / auth-bypass / SSRF / unsafe-deserialization? (`docs/operations/security-scanning.md`) | Builder + reviewer |
| **License compliance** *(when an SBOM is produced)* | Do dependency licenses pass policy — no denylisted copyleft; undetermined ones reviewed? (`scripts/license-check.sh`) | Builder + reviewer |
...
Same structure at every level (fractal); different scope and frequency. **Every retro must exit into an artifact** — that routing *is* the "adjust" step. A retro that changes nothing is theater. **Epic-close / release-boundary checklist** — the same rule binds the boundary event (an epic closing, a release train shipping): run the kit-steward meta-control panel at the **full (11-dimension) profile** (`docs/enterprise/meta-control.md`), which leaves the two standing artifacts, both **human-committed** — a row appended to `docs/governance/meta-control-log.md` and a dated verdict artifact under `docs/architecture/` — then route its findings into the backlog as ordinary rows and advance the state marker with that log row, so the freshness detector and the release rung see one record. The trigger is thereby a **documented boundary event**, not conductor self-start: a check that distrusts the conductor must not depend on the conductor to remember it. **The teeth do not move** — enforcement remains the release-tag cadence gate (ruling `D-240807-1`), which refuses a tag once the cadence is ESCALATED, and every tag additionally prints one line of panel freshness. **Ceiling:** this checklist is *conduct anchored in a governing doc*; a boundary that skips it is caught only later, by that responder, at >2N release tags.
...
**Promotion is proportional** — rigor at each promotion scales with **rung × change-class** (Ordinary / Sensitive / Control-plane; full matrix in [`docs/governance/promotion-contract.md`](docs/governance/promotion-contract.md)). Relaxation is **deferral, not a waiver**: the destination rung's gates fire in full on the whole accumulated change at every promotion, and the Control-plane class stays human-ratified at every rung.
...
- **Conflict resolution.** Defined precedence today: serialize board-touching PRs, and let parallel sessions work disjoint seams (the interim cross-session conventions). A re-sync procedure for genuinely overlapping parallel work was proposed and **rejected** by the `TRACKER-BACKED-GOVERNANCE` design sitting (mirroring the issue into the repo); a mechanism for that case is open Milestone B work, not yet built.
- **Review routing / ownership.** CODEOWNERS-style mapping of who/which agent/lens reviews what. **An agent never reviews-and-merges its own work.** A **validation / field-test** task under review carries a third terminal state (`skills/review` FAITHFUL-FAILURE) — a faithful test that surfaces a real defect is a test success (not NEEDS-FIXES); the defect routes to the originating backlog, never a repair in the vehicle. Ratification authority by role → §13 and `docs/enterprise/ratification-rbac.md`. **Seats before the push:** both seats — **reviewer and security** — are dispatched **against the working tree, BEFORE the push**, not after the PR exists, their verdicts and every fix round logged in the slice design's amendment log (narrative, committed, visible to the human on the PR); the reason is economic and measured — a finding that arrives after the PR opens has already cost a CI round trip, the most expensive way to learn it. **What ships, and its ceiling (rewritten 2026-09-04, `LOOP-STAGE-ARTIFACT-GATE`; this paragraph used to read "there is no machine-checked 'the seats ran' record, and none ships").** The old text accepted `D-240805-3` — a record an agent issues about its own conduct is written by the same actor whose conduct it attests — and concluded that any gate here would be vacuous. **The premise stands; the conclusion was wrong,** and the T1 harvest measured why: 7 of the 17 process defects nobody recorded shared one mechanism, and it was not that the record was self-attested — it was that **no record existed at all**, in any shape. Existence and shape are checkable, and there is one attesting actor who is **not** the builder: the non-author approver the merge already requires. So **what ships is `conformance/review-lane.sh` + the required `review-lane` context** — a sensitive/control-plane PR (or an ordinary, non-docs-only one) must carry `Kit-Plan:` and `Kit-Review:` trailers naming a tracked plan file and a review record, graded for Builder ≠ Reviewer, rounds citing commits inside the PR closing on APPROVE, every finding disposed in a grammar, every design-promised control resolving to an executable `path::test-name`, and a security-review verdict (or a waiver carrying all four `D-240904-1` criteria). **The non-self-attested half is the FORGE'S, not the check's** (amended 2026-09-05, `REVIEW-LANE-WAITING-IS-GREEN`): the approver types nothing and clicks Approve, and `required_approving_review_count >= 1` + `dismiss_stale_reviews` + `require_last_push_approval` enforce a non-author approval, bound to that exact head, from someone who did not push it — asserted live by `conformance/branch-protection.sh`. The check itself used to read the review list and return WAITING until an approval existed, which a job can only render RED and which only a manual re-run cleared; it is structural now, one run per push, green or red. **The ceiling, exactly:** it proves **shape, not truth** — that a record exists and is well-formed — and it proves nothing about who approved. It does not prove the reviewer read the diff, the same trust the merge already rests on. What changed is that **"never reviewed" is now RED** rather than invisible. Solo with one account the approval half is **unavailable** rather than weak, so adopters receive the check on an `observe` dial (`REVIEW_LANE_MODE`) whose exit condition is real record discipline. **Still not covered:** that the *seats* ran left of the push — a declared property of the flow, enforced by the human reading the design's amendment log; when it is skipped, seat findings still die left of CI and surface as red batteries.
...
| Merge to trunk | Recorded GO — a human judgment, on which the agent actuates (the promotion contract below); control-plane changes retain the kill-switch keystroke | Integration risk |
...
**Governed exceptions.** Required gates (§14 of the standards) and security posture are **universally required — never silently "conditional."** An exception is an auditable event: a **security-owner-ratified, time-boxed** record stating what is waived, why, the expiry, and the compensating control. → `docs/enterprise/ratification-rbac.md`. For **brownfield adoption**, this is operationalized as `templates/WAIVER-REGISTER.md` (validate it yourself with `sh conformance/waivers-valid.sh` — no shipped pipeline runs it for you; `secret-scan` and `branch-protection` are non-negotiable) — see `docs/adoption/brownfield.md` §5.

### Change-class & the promotion contract

The autonomy tiers answer *who acts*; the **change-class** — **Ordinary** / **Sensitive** / **Control-plane** (path-derived via `is_control_plane_path`; default to the higher class when uncertain) — answers *what's changing*. Together they set how much ceremony a change carries on the way to users: the **promotion contract** (`docs/governance/promotion-contract.md`). **GO/NO-GO is a recorded judgment, not a keystroke** (execution delegable post-GO — **now operative**: see the promotion contract's *Delegable execution* section — Ordinary/Sensitive delegable after a recorded GO; **control-plane judgment stays human; actuation is delegable on a SHA-bound, per-merge recorded GO**; `gh pr merge --admin` stays human). The **Control-plane class stays human-ratified at every rung** — never agent-self-governable; locked by `conformance/promotion-contract-documented.sh`. For **non-control-plane** promotions the **`approve→execute→log`** protocol makes the delegated keystroke auditable: the agent provides the means and waits, the human's GO is explicit/recorded/per-gate/**never inferred**, then the agent executes + logs (`scripts/promotion-verify.sh record` → a git note under `refs/notes/promotions` (view: `promotion-verify.sh log`)) + verifies **`shipped == approved`** at merge and tag (`conformance/promotion-verify-wired.sh` — wired live in the kit's own CI; it ships to you, and the portable aggregate runs its selftest). **`builder ≠ ratifier`** is a first-class invariant, peer to `builder ≠ reviewer`.

**The honest actuation model (S4/KW20).** The **GO is the only validation**; on the GO the agent *executes*; a human keystroke is *never a control* — only ever a **kill-switch** against a compromised/malfunctioning agent. The retained control-plane keystroke is honestly a **kill-switch + a temporary bootstrap**, not "human control" (ending it is a later slice, gated on recovery/rollback). **`lean` is the honest baseline** (agent actuates on a recorded GO, kill-switch off); **`enterprise` is the superset** that *adds* scaffolding — it strengthens, never removes an applicable gate. The kill-switch is a **posture, not a gate**: no enforcement-surface script reads the process mode (`conformance/mode-enforcement-blind.sh`). Full model: [`docs/governance/promotion-contract.md`](docs/governance/promotion-contract.md).
...
**The agent-boundary CI gate (harness-independent enforcement).** Every PR is checked by `conformance/agent-boundary.sh` (job: `control-plane-ratification`, in its own workflow `.github/workflows/ratification.yml` — the job KEY is the required status context): a diff touching a control-plane path (the set defined in `guard-core.sh::is_control_plane_path`) must carry a CODEOWNER (non-author) approval, or the gate **waits**. **Since 2026-08-28 a waiting gate is GREEN, with a notice** — because the thing that blocks the merge is the branch-protection **review requirement** (`required_approving_review_count ≥ 1`, server-side), not this check's colour, and a red that means "nothing is wrong" trains a team to stop reading reds. The check reds only where something is genuinely wrong: an approval is **present** and the gate still does not ratify (the approver is the author, the review list was unreadable, the change-class fail-safed), or the gate could not evaluate the diff. ⚠️ **Declared dependency:** the rendering is honest only while the review requirement exists — set `required_approving_review_count` to 0 and an unratified control-plane PR shows green with nothing blocking it; the setting and the check are one control. Greening the waiting state also cured a **worst-wins** defect: `pull_request` and `pull_request_review` both run the job on one head and GitHub keeps the worst conclusion, so a pre-approval red could outvote the post-approval green. (History: the waiting state was a yellow POSTED check-run until 2026-08-27 — deleted because branch protection stopped matching API-posted runs — then RED for a day.) An approval re-triggers the gate — and only the gate — which then confirms the ratification. Solo maintainers ratify via a logged `enforce_admins: false` admin-merge (`START-HERE.md`). The rule holds on every harness, including one with no inline guard. **The gate is adjudicated from the base tree and takes the changed-file listing from the PR files API — it never checks out or executes code from the PR it is judging, or a PR could rewrite the gate that judges it.** It is a §13 governance gate, not one of the 7 required build gates. Honest ceiling: CI is post-hoc; the real boundary remains platform-owned (`docs/enterprise/platform-safety-boundary.md`). The gate now also emits the honest team/solo SoD state label (`RATIFIED-BY-SECOND-REVIEWER` for a non-author approval, `SOLO-ADMIN-OVERRIDE-LOGGED` for a logged solo admin-override) and is change-class-aware (Slice 3) — a pre-merge projection surfacing the merge's SoD reality in plain language; it changes no solo behaviour. **Platform applicability.** This is a conditional gate — **Control-plane ratification** *(GitHub check-runs)* — built on GitHub check-runs and `pull_request_review`, which GitLab does not provide; on a GitLab host the same separation-of-duties obligation is met by the documented manual path (`docs/operations/gitlab-adoption.md`), so `conformance/proportional-gate-wired.sh` returns N/A-with-reason there rather than failing an adopter over an absent trigger, and `conformance/conditional-gates.sh` locks this declaration — those two are wired in the kit's own CI and ship to you to run on demand, not in an emitted pipeline. **Sibling gates (Phase-B slice B6):** `profiles/adopter-gates.yml` (Inception-installed, every stack, unconditionally) adds three more merge-time gates, each a job keyed with its own context name — `backlog-presence` (a gated PR binds a board row), `ceremony-binding` (a gated PR carries a recorded design GO), `loop-state` (the Entry-Declaration refusal floor, shipped ACTIVE with its `LOOP_STATE_MODE` dial defaulting to `enforce` since 2026-08-30 — the gate's own rc is the job's exit, so a missing or invalid declaration blocks the merge; set the dial to `observe` at the top of `.github/workflows/adopter-gates.yml` to opt out, in which case the job runs the gate, prints the verdict and always exits 0, and you should unbind the context in the same edit. **What the head owes is proportional to the change class:** an ordinary change owes `Kit-Row` + `Kit-Class`; a sensitive or control-plane change owes `Kit-Stage` and `Kit-Skill` as well) — same conditional shape, **Board and loop gates** *(GitHub check-runs)*, same GitLab manual-equivalent path (`docs/operations/gitlab-adoption.md`), locked by `conformance/adopter-gates-parity.sh` + `conformance/conditional-gates.sh`.
...
| Plan file `docs/plans/<date>-<row-slug>.md` (`templates/PLAN-RECORD-TEMPLATE.md`) — named by the head's `Kit-Plan:` trailer; carries the design-promised controls forward | Plan | the slice's task list or promised controls change | planning agent |
| Code + tests | Build | every change | building agent |
| Review record `docs/reviews/<date>-<row-slug>.md` (`templates/REVIEW-RECORD-TEMPLATE.md`) — named by the head's `Kit-Review:` trailer, **appended per review round**. `review-lane` grades the record; the non-author Approve beside it is enforced by branch protection, not by that check (`REVIEW-LANE-WAITING-IS-GREEN`, 2026-09-05) | Review (first round) | every round, and every fix push | reviewing agent (record) + non-author approver (the Approve, typing nothing) |
