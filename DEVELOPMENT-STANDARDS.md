...
### Neutrality by construction (standing requirement)

- **Comparable cards** — every option recorded with the same fields, side by side (no favourite gets a richer entry).
- **Fit-driven selection** — the choice is derived from the problem's dimensions, not from familiarity or reputation.
- **Fit-vs-maturity disclosure** — the recommendation states both the best-fit option *and* its maturity tier; the owner ratifies the trade-off explicitly.
- **Anti-bias lint** — a conformance check on the decision artifact rejects bias-appeal ("it's the proven default") and requires a cited fit reason.

**Every concretization axis MUST carry this** (proven-ness is not a justification) — recipe, next-axis checklist, and card template in [docs/adoption/neutrality-by-construction.md](docs/adoption/neutrality-by-construction.md). Worked: stack ([STACK-SELECTION](docs/STACK-SELECTION.md), `conformance/stack-decision-integrity.sh`) · deploy-target ([DEPLOYMENT-ENVIRONMENT](docs/adoption/DEPLOYMENT-ENVIRONMENT.md), `conformance/deploy-decision-integrity.sh`) · harness ([harness-adapters](docs/operations/harness-adapters.md), `conformance/harness-decision-integrity.sh`); model subsumed by harness (not a separate axis).
...
### Script self-disclosure

Every human-run kit/repo script carries a plain-English header disclosing **what it does · what it changes · guardrails** (the security/governance constraints it requires or refuses), enforced by `conformance/script-disclosure.sh`. **Pinned grammar:** the header's first line states purpose (`# <name> — <purpose>`), plus a `# What it changes:` line (the files/state it mutates, or "read-only") and a `# Guardrails:` line (what it requires or refuses). Sourced-only helper libraries (never run by a human) are allowlisted; the gate proves the two lines are present, not that they are accurate — review is the backstop.
...
### Versioning
- **Versioned** from day one (e.g. `/v1`); the version is explicit in the contract.
- **Additive changes only** within a version — new fields never break existing consumers; never remove or rename response fields silently.
- **Breaking changes** ship as a **new version**, never in place, with a deprecation window on the prior one.

### Deprecation
- **Announce** the deprecation first, **maintain** the old surface for a **stated minimum period**, then **remove only on a MAJOR version bump** with a **documented migration path**. The concrete window and mechanism are a **→ profile** decision.
...
| 5 | **Secret scan** | The full commit HISTORY is scanned for committed secrets on every push (`gitleaks git .`, `fetch-depth: 0`); any finding fails the build. A secret committed and later removed is still caught — the scan is not limited to the working tree at HEAD. `.gitleaks.toml` allowlists only the promotion-note `approval-token: "…"` free-text field (a line-targeted regex, never a path exclusion). |
...
**Conformance:** a tree's **own installed pipeline** is verified by the non-kitself leg `sh conformance/ci-gates.sh --own-tree` (a registered `verify.sh` control row every tree runs; `ci-gates.sh <workflow>` verifies a single reference workflow the same way), which asserts every required gate is declared (the Definition-of-Done "CI/CD" check, `CLAUDE.md`). The id set is the **8 standardized gate-ids implementing the seven required gates — gate 7 (supply-chain) expands to `gate-sbom` + `gate-provenance`**. A tree may carry a decided per-gate disposition (`conformance/gate-dispositions.txt`, na-with-reason, validated line-by-line; **absent = all 8 required**, the adopter default); the kit's own meta-CI runs under its `D-240805-2` disposition (3 apply · 5 na-with-reason, ruling-locked by the leg's selftest). Gates are declared **by id on any CI platform** — GitHub Actions `id: gate-X` steps, GitLab CI `gate-X:` job keys, or a documented mapping for other platforms — because the contract is the gate-ids, not a vendor. See `docs/operations/ci-platforms.md`.

**Gate authority — the two suites, and which is authoritative.** Quality gates run in two places: the portable conformance aggregate (`sh conformance/verify.sh --require`) and your emitted CI pipeline. **Your pipeline MUST run the aggregate** — every pipeline the kit emits carries it as an early blocking step, before any toolchain setup, on GitHub and GitLab alike (POSIX `sh`, no toolchain dependency, so it fails fast). Without it the two suites disagree silently and a project goes **remote-green while locally-red on a required gate**; enforced by `conformance/verify-enforced-wired.sh`, so keep the step if you rewrite your pipeline. "Blocking" covers the **control** checks — `[doc]` failures are reported but exit 0 by design, so a day-one project is not blocked on an unwritten RPO/RTO. The contract is the *invocation*, not the vendor: GitHub `run:`, GitLab `script:` (same rationale as the gate-ids rule below).

**Named exclusions.** Some checks are meaningful only in the kit's own repo — they lock its release mirror, tag protection, and CI shard matrix — and would correctly fail in yours, so they are enforced as workflow steps rather than in the portable battery. Every conformance check is **registered** in `verify.sh`, **wired** as a workflow step, or **declared a library** in `conformance/aggregate-exclusions.txt`; `conformance/aggregate-coverage.sh` (kit-only) fails if any is in none, which is what stops one suite gaining a required gate the other lacks. **Honest ceiling:** the kit runs the aggregate against **real incepted projects** in its own CI — a greenfield GitHub tree, a GitLab tree, and a brownfield tree whose foreign pipeline is preserved — while the other nine profiles and the GitLab pipeline's other stacks are verified **statically**, present and correctly shaped, not observed to run; the gate-id contract itself is now judged **dynamically on each tree's own installed pipeline** (`ci-gates.sh --own-tree`), but an id is a declaration — execution is still not attested. Suppression detection (`verify-enforced-wired.sh`) rejects **known** forms only — an enumeration, not a decision procedure — so that class is not closed.

**CI security hardening (required posture, not a gate).** The provenance/attestation step requires `id-token: write`; grant it via a **separate job that runs only on push-to-main**, keeping the main gate job at `contents: read` so PR-triggered steps cannot mint an OIDC token a poisoned dependency could exfiltrate. Pin third-party actions to a full commit SHA in production (keep the SHAs current with Dependabot, which updates the SHA and its `# vX` comment together); the canonical reference `profiles/typescript-node/ci.yml` models this and is enforced by `conformance/action-pinning.sh` in the kit's own CI (the script ships to you; run it on demand over your own workflows). The cloud OIDC trust policy **MUST** restrict `sub` to the main-branch ref (`refs/heads/main`), never `pull_request`. The profile reference pipelines model this two-job split.
...
They are deliberately **not** universal required gates: forcing a conditional gate (say a11y, load, or eval) on a CLI, library, or batch job that has no UI, no service, and no model would be false universality. Verified by `conformance/conditional-gates.sh` in the kit's own CI (the script ships to you; run it on demand — no emitted pipeline wires it).

**First-run-green manifests — assert the ENFORCING VALUE, not mere presence.** A manifest block that gates a security mechanism (the `incept-first-run-green` detector; `conformance/incept-manifests/<stack>.txt`) MUST assert the mechanism's *enforcing value* — a threshold, a deny-flag, a severity floor — not merely that the step is present. A present-but-inert gate satisfies a presence-only assert while never failing the build: e.g. an OWASP dependency-check with `failBuildOnCVSS` at its plugin default of `11` runs on every push yet can never fail (CVSS ≤ 10), so `assert: present …dependency-check…:check` greens a gate that gates nothing. Assert the value instead — `assert: present -DfailBuildOnCVSS[[:space:]=]*7` (or the scaffold-side `failBuildOnCVSS = 7.0f`). Model it on the reference manifests' security blocks (e.g. `sast-scoped`'s `absent --config auto` + whole-tree `--error .` scope). **Honest ceiling:** author discipline — the grammar supports value-asserts but does not enforce their use; the reference manifests are the exemplars and review is the backstop. (Surfaced by the KW8-followon run, 2026-07-08.)
...
- on release, carry **(a) an image SBOM** (CycloneDX, generated by scanning the built image — e.g. Syft) **and (b) a build-provenance attestation bound to the image digest** (`actions/attest-build-provenance` with `subject-name` + `subject-digest`, and `push-to-registry: true`), not merely the language artifact or a mutable tag; and **(c) an image vulnerability scan** (Trivy) that fails the gate on fixable CRITICAL/HIGH (`gate-image-vuln`); unfixed CVEs are enumerated by the SBOM, not gated.
