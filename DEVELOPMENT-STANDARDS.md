...
**Named exclusions.** Some checks are meaningful only in the kit's own repo — they lock its release mirror, tag protection, and CI shard matrix — and would correctly fail in yours, so they are enforced as workflow steps rather than in the portable battery. Every conformance check is **registered** in `verify.sh`, **wired** as a workflow step, or **declared a library** in `conformance/aggregate-exclusions.txt`; `conformance/aggregate-coverage.sh` (kit-only) fails if any is in none, which is what stops one suite gaining a required gate the other lacks. **Honest ceiling:** the kit runs the aggregate against **real incepted projects** in its own CI — a greenfield GitHub tree, a GitLab tree, and a brownfield tree whose foreign pipeline is preserved — while the other nine profiles and the GitLab pipeline's other stacks are verified **statically**, present and correctly shaped, not observed to run. Suppression detection (`verify-enforced-wired.sh`) rejects **known** forms only — an enumeration, not a decision procedure — so that class is not closed.
