...
A complete, **stack-agnostic** software development lifecycle designed for teams working with AI agents — from idea through released, operating software. Drop it into a new project, choose your stack, and run `incept` — for a service stack it scaffolds a runnable starter wired to CI plus the environments your stack needs, so you start from a runnable, CI-wired starter, not an empty repo. (Green-on-clone scope is honest per stack — see the Quickstart.)
...
4. Run **`scripts/incept.sh`** — for a **service stack** it scaffolds a runnable starter (a `/healthz` service + test wired to the stack's CI), writes `.env.example`, and copies the stack's `compose.yaml`. **Green-on-clone scope, honest per stack:** `typescript-node`'s **language gates** (install → lint → type-check → test+coverage → build) are maintainer-verified green on clone, and incept ships its `Dockerfile` + `compose.yaml` so the supply-chain/image-build gates run on your CI too (those weren't maintainer-executed); the other service stacks (`python`, `go`, `rust`, `java-spring`, `kotlin`, `dotnet`) need a **one-time lockfile/wrapper step** (see each `scaffold/README.md`) and have not been maintainer-executed; **`ml` / `data-engineering` / `terraform`** ship a CI contract you populate (no `/healthz` starter — first push is red until you add source).
